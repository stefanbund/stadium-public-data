...
## 20. Trader Logging & Intelligence Architecture [NEW]

The Hierarchical Trader manages a distributed logging system designed to provide 100% auditability for every neural network decision, regardless of whether a trade is physically executed.

### 20.1 Unified Prediction Logs (The Intelligence)
- **Path**: `/STADIUM_DATA/SCP-PRED/{SYMBOL}-unified-prediction-log-{timestamp}.csv`
- **Role**: This is the "Why" behind every trade. It records the complete internal state of the 5-layer Bayesian logic at the moment a signal is processed.
- **Contents**: Preserves the binary votes and metadata for Directional, Imbalance, Crash, Generalist, and Specialist tiers. These files are generated even if the trader is in "Signal Only" mode.

### 20.2 Execution Ledger (The Results)
- **Path**: `UNIFIED_TRADER_WORKSPACE/JS-TRADER-REWRITE/executions_log.csv`
- **Role**: A chronological source-of-truth for all live trade attempts. This ledger is the primary data source for the Operations Dashboard.
- **Failure Resilience**: The system now implements an "Always-Record" policy. If a trade is rejected due to `Insufficient Funds` or a `400 Bad Request` from Coinbase, the failure reason is explicitly logged as a `FAILED` cycle, allowing for retrospective analysis of the NN's decision "wisdom" despite zero balance.

### 20.3 Trader Runtime Logs (Process Telemetry)
- **Path**: `UNIFIED_TRADER_WORKSPACE/logs/trader_restart.log` (or `trader_runtime.log`)
- **Role**: High-level system health monitoring.
- **Contents**: Captures continuous "Heartbeat" messages, cycle processing speeds, and infrastructure anomalies (e.g., file descriptor counts or virtual memory warnings).

### 20.4 Operations Dashboard (The Human View)
- **Path**: `UNIFIED_TRADER_WORKSPACE/operations_dashboard.html`
- **Role**: A premium, visual UI that compiles the raw CSV execution data into interactive charts.
- **Metric Tracking**: Surfaces Total Triggers, Successful Clears, and execution rates, providing a "Red/Green" status for every decision window.

### 20.5 ARIMAX Forecast Logs (Secondary Analysis)
- **Path**: `arimax_forecaster.log`
- **Role**: Detailed diagnostics for the ARMAX-GARCH statistical engine. Used primarily for debugging the precision of the price ceiling and floor projections.
